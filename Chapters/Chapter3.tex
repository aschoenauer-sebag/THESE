...
Six distances were selected to compare phenotypic profiles following drug exposure. We then wanted to evaluate their ability to distinguish between different conditions\footnote{A condition is the combination of a drug and a dose.} without distinguishing between condition replicates. For this purpose, we computed for each distance $d$ and condition $C$ the following quantities: for each experiment $i$ we note $C_i$ its condition, $corr$ the Pearson correlation and $d(i,M)$ the vector of distances between $i$ and all Mitocheck hit experiments
\begin{equation}
S_d(C)=\dfrac{\sum_i \sum_{\substack{k|C_k\neq C}} d(i,k)}{\sum_i \sum_{\substack{j| C_j = C, j\neq i}} d(i,j)}
\end{equation}

\begin{equation}
R_d(C)=\dfrac{2}{(n-1)(n-2)}~ \sum_i \sum_{\substack{j\\ C_j = C_i\\ j\neq i}} corr(d(i,M), d(j,M))
\end{equation}
